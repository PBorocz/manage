...
#+end_src
*** Methods
We provide the following methods (listed alphabetically)
**** build
Builds a package distribution using Poetry, ie. `poetry build`.

There are *no* optional arguments to this command.
...
#+end_src
**** check
This can be directly invoked from the command-line
...
#+end_src
**** ~git_add~
Performs a `git add <pathspec>` operations.
  #+begin_src yaml
    build_my_package:
      description: Build my distribution package.
      steps:

...
** Development
   - Confirm python version availability, I'm developing on 3.10.9 for now (and use ~pyenv~ to manage all my versions)
   ...
   - Set ~.envrc~ to point top-level directory (i.e. README and pyproject.toml) (thus, I use ~direnv~). Here's why my ~.envrc~ contains:
#+begin_src bash

...
#+end_src
** Assumptions
To the degree that your development/project/release environment strays from mine, the tool probably becomes less and less relevant!

However, these are my common python project standards, allowing for the ability of this tool to work seamlessly:
*** Tools
...
- We assume ~README.org~ / ~README.md~ is kept in the top-level directory.
...
| ~update_readme~                  | Yes           | Yes        | ~readme_format~     |
|--------------------------------+---------------+------------+-------------------|
**** Details
***** *build*
...
****** Arguments
- ~readme_format~ Required, a string that is one of ~org~ or ~md~.

****** README files can be in one of two formats:
******* ORG (.org) Format
A supported README in org format might be:
 #+begin_src org
   ,* My Project
   ,** Stuff...
   ,** More Stuff...
...
#+end_src
******* Markdown (.md) Format
A supported README in markdown format might be:
 #+begin_src markdown
   # My Project

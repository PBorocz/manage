...
** GTD
- [TODO] Fix bad language in message when only a single target is defined in a yaml file (e.g. "must be one of  or deploy").
- [TODO] Loosen up the requirement for a "packages" entry in pyproject.toml for projects that AREN'T building/deploying packages!
